...
export class Index extends SampleIndexBase {
	...
	configureRouter(config: RouterConfiguration, router: Router) {
		const routes: RouteConfig[] = [
			...
			super.getRouteConfig("textarea"),
			super.getRouteConfig("input-types"),
			super.getRouteConfig("html5-validation"),
			super.getRouteConfig("aurelia-validation"),
			super.getRouteConfig("character-counter"),
			super.getRouteConfig("icon-prefixes"),
			super.getRouteConfig("placeholder"),
		];
		config.map(routes);
		this.router = router;
	}
